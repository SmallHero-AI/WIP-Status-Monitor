...
"""
================================================================================
  V8.5 全自動化更新、選股、訊號推播與 Git 推送管線 (update_and_push_v8_5.py)
================================================================================
"""
import os
import json
...
def main():
    print("=" * 60)
    print("  [Start] V8.5 全自動化更新與推送管線 (update_and_push_v8_5.py)...")
    print("=" * 60)
    ...
    if os.path.exists(LEADERBOARD_PATH):
        try:
            ...
            print(f"⚠️  讀取 leaderboard_v8_5.json 失敗: {e}")
    ...
    # 4. 執行高勝率型態策略重新篩選 (V8.5)
    print("🚀 4. 執行高勝率型態策略重新篩選 V8.5 (search_75_winrate_v8_5.py)...")
    try:
        subprocess.run(["python", "search_75_winrate_v8_5.py"], cwd=SCRIPT_DIR, check=True)
        print("✅ 4. 型態策略篩選與排行榜重建完成。")
    except Exception as e:
        print(f"❌  型態策略篩選與排行榜重建失敗: {e}")
    print("-" * 60)

    # 5. 重新生成精選個股回測 Excel (V8.5)
    print("🚀 5. 重新生成精選個股回測 Excel V8.5 (export_high_win_excel_v8_5.py)...")
    try:
        subprocess.run(["python", "export_high_win_excel_v8_5.py"], cwd=SCRIPT_DIR, check=True)
    ...
    # 6b. 自動發送今日最新觸發訊號至 LINE 廣播
    print("🚀 6b. 發送今日觸發訊號至 LINE 自動推播 (send_signal_notifications_v8_5.py)...")
    try:
        subprocess.run(["python", "send_signal_notifications_v8_5.py"], cwd=SCRIPT_DIR, check=True)
        print("✅ 6b. 今日訊號 LINE 廣播推播完成！")
    except Exception as e:
        print(f"❌  今日訊號推播失敗: {e}")
    print("-" * 60)

    # 7. 自動提交與推送 Git 倉庫
    print("🚀 7. 自動提交與推送 Git 倉庫 (V8.5)...")
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8.5 Auto Update: 啟用三態訊號推播與 V8.5 獨立型態回測管線(Server)"], cwd=server_dir, check=True)
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8.5 Auto Update: 啟用三態訊號推播與 V8.5 獨立型態回測管線(Root)"], cwd=root_dir, check=True)
    ...
    print("=" * 60)
    print("  [Done] V8.5 全自動化更新、選股、推播管線執行完畢！")
    print("=" * 60)

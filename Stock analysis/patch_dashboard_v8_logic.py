...
HTML_PATH = r"E:\G-AI-1\Stock analysis\修正版_V6_Server\public\dashboard_v8_backup.html"
...
def main():
    ...
    new_holding_summary = """        <!-- 📈 策略進出場成績總覽頁面 (V8) -->
        ...
        <!-- 📊 持倉總覽面板 -->"""
    if old_holding_summary in html and "tab_overview" not in html:
        html = html.replace(old_holding_summary, new_holding_summary)
    ...
    new_load_leaderboard = """        async function loadLeaderboardData() {
            try {
                backtestRegistry = {};
                activeHoldings = {};
    ...
    if old_load_leaderboard in html:
        html = html.replace(old_load_leaderboard, new_load_leaderboard)
        print("  [成功] 修改 loadLeaderboardData 支援 V8 雙向格式")
    else:
        # 如果格式有微小差異，做精準替換
        html = html.replace("const res = await fetch('/leaderboard.json');", "const res = await fetch('/leaderboard_v8.json');")
    ...
    new_v8_functions = """        // V8 交易方向與成績總覽邏輯

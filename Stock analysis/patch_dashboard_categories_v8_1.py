...
HTML_BASE_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_1_backup.html")
HTML_OUT_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_1.html")
...
def main():
    print("=" * 60)
    print("  AI 策略分類網頁編譯工具 V8 (Idempotent 雙向升級版)...")
    print("=" * 60)
    ...
    long_rebound = ["雙重超賣 (RSI+CCI)+MACD轉強", "CCI超賣+MFI超賣+KD金叉"]
    ...
    short_trend = [
        ...
        "空頭拉回：MA60之下+KD死叉+RSI>55", "動能共振：EOM下跌+MACD<0+RSI<50",
        "支撐跌破：價格<S1+EOM弱勢+MACD<0", "長期均線阻力：跌破MA120+KD死叉+CCI超買"
    ]
    short_rebound = ["雙重超買 (RSI+CCI)+MACD轉弱", "CCI超買+MFI超買+KD死叉"]

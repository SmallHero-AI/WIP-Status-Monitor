...
"""
================================================================================
  V8.5 精選高勝率個股 Excel 回測生成工具 (export_high_win_excel_v8_5.py)
================================================================================
"""
import os
import glob
...
def get_strategy_signals(df, ent_name, ext_name, is_short=False):
    ...
    if not is_short:
        ...
        elif ext_name == "跌破MA20或6%停損": sl, ext_sig_raw = 0.06, f_below_ma20
    ...
    for i in range(1, n_rows - 1):
        if not hold:
            ...
            if not is_short:
                ...
                if ext_name == "波段追蹤：ATR追蹤回補 (最低價拉回3*ATR離場)":
...
def main():
    ...
    print(f"[資訊] 共有 {len(high_win)} 檔股票勝率 >= 75% (V8.5)")
    ...
    for item in high_win:
        ...
        ws_summary['A1'] = "V8.5 高勝率量化分析回測參數與績效總覽"
        ...
        col_R = get_column_letter(n_base_cols + 1)
        col_S = get_column_letter(n_base_cols + 2)
        col_T = get_column_letter(n_base_cols + 3)
        col_U = get_column_letter(n_base_cols + 4)
        col_V = get_column_letter(n_base_cols + 5)
        col_W = get_column_letter(n_base_cols + 6)

        ws_data['A1'] = "個股交易回測明細 (V8.5)"
        ...
        excel_name = f"{code}_AI_v8_5.xlsx"

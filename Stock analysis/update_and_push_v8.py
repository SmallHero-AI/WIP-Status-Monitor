...
def main():
    print("=" * 60)
    print("  [Start] V8 全自動化更新與推送管線 (update_and_push_v8.py)...")
    print("=" * 60)
    ...
    # 4. 執行高勝率策略重新篩選 (V8 雙向)
    print("🚀 4. 執行高勝率策略重新篩選 V8 (search_75_winrate_v8.py)...")
    try:
    ...
    # 5. 重新生成精選個股回測 Excel (V8 雙向)
    print("🚀 5. 重新生成精選個股回測 Excel V8 (export_high_win_excel_v8.py)...")
    try:
    ...
    # 6. 編譯網頁 (V8)
    print("🚀 6. 編譯並更新網頁 V8 (patch_dashboard_categories_v8.py)...")
    try:
        subprocess.run(["python", "patch_dashboard_categories_v8.py"], cwd=SCRIPT_DIR, check=True)
        print("✅ 6. 網頁編譯與注入完成。")
    except Exception as e:
        print(f"❌  網頁編譯與注入失敗: {e}")
    print("-" * 60)
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8 Auto Update:行情與雙向策略數據自動更新(Server)"], cwd=server_dir, check=True)
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8 Auto Update:行情與雙向策略數據自動更新(Root)"], cwd=root_dir, check=True)

...
LEADERBOARD_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "leaderboard_v8_4.json")
...
def main():
    print("=" * 60)
    print("  [Start] V8.4 全自動化更新與推送管線 (update_and_push_v8_4.py)...")
    print("=" * 60)
    ...
    if os.path.exists(LEADERBOARD_PATH):
        try:
            ...
            print(f"⚠️  讀取 leaderboard_v8_4.json 失敗: {e}")
    ...
    # 4. 執行高勝率型態策略重新篩選 (V8.4)
    print("🚀 4. 執行高勝率型態策略重新篩選 V8.4 (search_75_winrate_v8_4.py)...")
    try:
        subprocess.run(["python", "search_75_winrate_v8_4.py"], cwd=SCRIPT_DIR, check=True)
        print("✅ 4. 型態策略篩選與排行榜重建完成。")
    except Exception as e:
        print(f"❌  型態策略篩選與排行榜重建失敗: {e}")
    print("-" * 60)

    # 5. 重新生成精選個股回測 Excel (V8.4)
    print("🚀 5. 重新生成精選個股回測 Excel V8.4 (export_high_win_excel_v8_4.py)...")
    try:
        subprocess.run(["python", "export_high_win_excel_v8_4.py"], cwd=SCRIPT_DIR, check=True)
    ...
    try:
        subprocess.run(["python", "patch_dashboard_categories_v8_4.py"], cwd=SCRIPT_DIR, check=True)
        print("✅ 6. 網頁編譯與注入完成。")
    except Exception as e:
        print(f"❌  網頁編譯與注入失敗: {e}")
    print("-" * 60)
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8.4 Auto Update: 行情與型態回測策略數據自動更新(Server)"], cwd=server_dir, check=True)
    ...
    try:
        ...
        if status.stdout.strip():
            subprocess.run(["git", "commit", "-m", "V8.4 Auto Update: 行情與型態回測策略數據自動更新(Root)"], cwd=root_dir, check=True)

...
"""
================================================================================
  V8.5 AI 策略分類網頁編譯工具 (patch_dashboard_categories_v8_5.py)
================================================================================
"""
import json
import os
...
HTML_BASE_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_5_backup.html")
HTML_OUT_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_5.html")
...
def main():
    print("=" * 60)
    print("  AI 策略分類網頁編譯工具 V8.5 (多尺度自適應型態策略版)...")
    print("=" * 60)
    ...
    for item in high_win:
        ...
        tp = 0.0
        sl = 0.0
        if "6%停利 / 6%停損" in exit_cond: tp, sl = 6.0, 6.0
        elif "8%停利 / 8%停損" in exit_cond: tp, sl = 8.0, 8.0
        elif "12%停利 / 6%停損" in exit_cond: tp, sl = 12.0, 6.0
        elif "KD" in exit_cond or "死叉" in exit_cond or "金叉" in exit_cond: sl = 6.0
        elif "RSI" in exit_cond: sl = 6.0
        elif "MACD" in exit_cond: sl = 6.0
        elif "MA20" in exit_cond: sl = 6.0
        elif "布林" in exit_cond: sl = 6.0
        elif "支撐" in exit_cond or "壓力" in exit_cond: tp = 8.0

        actual_filename = f'{code}_AI_v8_5.xlsx'
        ...
        new_stocks.append({
            ...
            'sl': sl,
            'type': strategy_type,
            'signalInfo': item.get('signalInfo')
        })
    ...
    for code in base_codes:
        matches = glob.glob(os.path.join(DATA_DIR, f"{code}_*.xlsx"))
        if matches:
            matches.sort(key=os.path.getmtime, reverse=True)
    ...
    if not os.path.exists(HTML_BASE_PATH):
        # 若無 backup，從 dashboard_v8_4.html 複製一份過來
        v84_path = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_4.html")
        if os.path.exists(v84_path):
            import shutil
            shutil.copy2(v84_path, HTML_BASE_PATH)
    ...
    if start_stocks_idx != -1 and end_stocks_idx != -1:
        ...
        print("  [成功 V8.5] 替換 preloadedStocks")
    ...
    if start_xlsx_idx != -1 and end_xlsx_idx != -1:
        ...
        print("  [成功 V8.5] 替換 xlsxFiles")

    with open(HTML_OUT_PATH, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"  [成功 V8.5] 儲存至 {HTML_OUT_PATH}")
    print("=" * 60)

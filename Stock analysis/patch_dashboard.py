...
def main():
    ...
    if start_stocks_idx != -1 and end_stocks_idx != -1:
        ...
        print("  [警告] 找不到 preloadedStocks 區塊")

    # 替換 xlsxFiles (找 const xlsxFiles = { ... };)
    ...
    if start_xlsx_idx != -1 and end_xlsx_idx != -1:
        ...
        print("  [警告] 找不到 xlsxFiles 區塊")

    # ── 2. 側邊欄 HTML 注入 ──
    # 尋找 v38 strategy-folder 區塊的結尾
    v38_end_pattern = """            <!-- V38 Folder -->
            <div class="strategy-folder" id="folder_v38">
                <div class="folder-header" onclick="toggleFolder('v38')">
                    <div class="folder-title">
                        <span class="folder-icon">▼</span>
                        <span>📁 鎖利系統 - V38 穩定版</span>
                    </div>
                    <div class="folder-actions">
                        <span class="lock-icon" style="display:none; font-size:0.8rem;">🔒</span>
                        <button class="btn-add-stock" onclick="triggerFolderUpload(event, 'v38')">➕ 載入</button>
                    </div>
                </div>
                <div class="folder-contents" id="contents_v38"></div>
            </div>"""
    
    # 由於排版可能有些許差異，用一行更精確地找 </div id="contents_v38"></div>
    # 或者就搜尋 <div class="strategy-folder" id="folder_v38"> ... </div> (包含整個 folder 區塊)
    v38_target_end = 'id="contents_v38"></div>\n            </div>'
    ai_folder_html = """\n
            <!-- AI Folder -->
            <div class="strategy-folder" id="folder_ai">
                <div class="folder-header" onclick="toggleFolder('ai')">
                    <div class="folder-title">
                        <span class="folder-icon">▼</span>
                        <span>📁 AI 交叉組合策略 - 穩定版</span>
                    </div>
                    <div class="folder-actions">
                        <span class="lock-icon" style="display:none; font-size:0.8rem;">🔒</span>
                        <button class="btn-add-stock" onclick="triggerFolderUpload(event, 'ai')">➕ 載入</button>
                    </div>
                </div>
                <div class="folder-contents" id="contents_ai"></div>
            </div>"""
    
    if v38_target_end in content:
        content = content.replace(v38_target_end, v38_target_end + ai_folder_html)
        print("  [成功] 注入 AI Folder HTML")
    else:
        # 降級嘗試使用單行包含 contents_v38 的替代尋找
        v38_fallback = 'id="contents_v38"></div>'
        if v38_fallback in content:
            content = content.replace(v38_fallback, v38_fallback + '\n            </div>' + ai_folder_html)
            print("  [成功] 注入 AI Folder HTML (降級模式)")
        else:
            print("  [警告] 找不到 folder_v38 結尾")
    ...
    render_replace = """document.getElementById('contents_v38').innerHTML = '';
            const contentsAi = document.getElementById('contents_ai');
            if (contentsAi) contentsAi.innerHTML = '';"""
    content = content.replace(render_target, render_replace)
    print("  [成功] 更新 renderSidebarStocks")

    # ── 4. selectStockTab 更新 ──
    # 允許 preload 型別若無 DOM 則動態生成其頁面
    ...
    # 執行回測更新：若 strategy 為 ai，優先執行 run_AI
    ...
    # ── 5. cleanCode 與名稱對照更新 ──
    # 更新 getStockName 讓其完全動態讀取 preloadedStocks
    ...
    # 更新 getStrategyDisplayName 讓其支援 'ai' 策略
    ...
    get_strat_display_replace = """        function getStrategyDisplayName(code) {
            ...
            if (stock) {
                ...
                if (stock.strategy === 'ai') return 'AI 交叉組合策略';
            }"""
    content = content.replace(get_strat_display_target, get_strat_display_replace)

    # 更新 updateUI 中的 cleanCode 提取 (移除 _ai 尾碼，對齊即時股價 API)
    ...
    # ── 7. createCustomStockPage 支援 ai 策略 ──
    # 在 2750 行 V38 後面加上 ai 條件
    ...
    if create_page_v38_end in content:
        ...
        print("  [成功] 更新 createCustomStockPage")
    else:
        print("  [警告] 找不到 createCustomStockPage 中 V38 策略結尾標記")

    # ── 8. 注入 run_AI 核心回測引擎 ──
    # 尋找 function run_V38 之前的合適注入點，比如在 run_V38 結尾後注入
    run_v38_end_target = """            updateUI(code, trd, tot, mCap, ds, bi, si);
        }"""
    
    # 由於 updateUI(code, trd, tot, mCap, ds, bi, si); 在多個策略結尾都有，我們找 run_V38 結尾
    # 我們可以直接搜尋 `function run_V38(code) { ... }` 之後的 updateUI
    # 為了安全與唯一性，我們可以直接在 script 區塊最後面，或 updateUI 之前注入 run_AI
    # 讓我們定位 `function updateUI` 並在它前面注入 `function run_AI(uniqueId) { ... }`
    run_ai_js = """
        // --- V5 新增: AI 交叉指標組合回測引擎 ---

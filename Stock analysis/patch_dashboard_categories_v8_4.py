...
HTML_BASE_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_4_backup.html")
HTML_OUT_PATH = os.path.join(SCRIPT_DIR, "修正版_V6_Server", "public", "dashboard_v8_4.html")
...
def main():
    ...
    for item in high_win:
        ...
        # 找尋實際存在的檔案
        actual_filename = f'{code}_AI_v8_4.xlsx'
        pattern = os.path.join(DATA_DIR, f'{code}_AI_*.xlsx')
        matches = glob.glob(pattern)
        if matches:
            # 排序讓最新的檔案 (例如 v8_4) 排在前面
    ...
    for code in base_codes:
        ...
        if matches:
            # 排序讓最新的檔案 (例如 v8_4) 排在前面，可以用修改時間或名稱降冪
            matches.sort(key=os.path.getmtime, reverse=True)
            # 優先找 AI 版本
